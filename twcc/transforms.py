...
def split_labels(mask, labels):
    """input torch 5D tensor (N, C, D, H, W), dtype=bool"""
    label_vols = []
    for label in labels:
...
class RandomCrop():
    ...
    def __call__(self, image):
        ...
        crop_size = (self.size,) * 3 if type(self.size) == int else self.size
...
class NonlinearDeformTio():
    ...
    def __call__(self, label):
        device = label.device
        data = tio.RandomElasticDeformation(
...
class RandomDownSample():
    ...
    def __call__(self, image):
        ...
        image = F.interpolate(input=image_lr, size=origin_shape, mode='trilinear') # upsample back to r_hr
        return image
...
class RandomDownSample_forDEMO():
    ...
    def __call__(self, image):
        ...
        image_lr = F.interpolate(input=image,  size=sample_shape, mode='trilinear') # downsample to low resolution r_spac
        image = F.interpolate(input=image_lr, size=origin_shape, mode='trilinear') # upsample back to r_hr
